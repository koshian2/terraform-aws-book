...
# ---- EC2（パブリックサブネット。公開IPなし、SSM接続）---- / EC2 (public subnet.no public IP, SSM access)
resource "aws_instance" "ssm" {
  ...
  associate_public_ip_address = false # ここを意図的に付与しないでエラーにする / Do not assign this intentionally to cause an error.
  ...
  tags = {
    Name = "${var.vpc_name}-ec2-ssm-no-public-ip"
  }
}
