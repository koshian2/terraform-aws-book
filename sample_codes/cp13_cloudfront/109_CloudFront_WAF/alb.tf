...
# --- HTTPリスナー (80) ：デフォルトは 403 応答に --- / HTTP listener (80): return 403 by default
resource "aws_lb_listener" "http" {
  load_balancer_arn = aws_lb.alb.arn
  port              = 80
  protocol          = "HTTP"

  # デフォルトは403を返す（秘密ヘッダーが無いアクセスはここに落ちる） / Return 403 by default. Access without the secret header reaches this rule.
  default_action {
    type = "fixed-response"
    fixed_response {
...
# --- 秘密ヘッダー一致時のみ フォワードする ルール（優先度1）--- / Rule that forwards only when the secret header matches, priority 1
