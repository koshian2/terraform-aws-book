...
locals {
  lambda_functions = [
    "lambda_function_hiragana",
    "lambda_function_katakana"
  ]
}
...
resource "aws_sfn_state_machine" "state_machine" {
  name     = "ParallelLambdaStateMachine"
  role_arn = aws_iam_role.step_functions_role.arn

  definition = templatefile("${path.module}/state_machine_definition.json.tftpl", {
    lambda_hiragana_arn = aws_lambda_function.lambda_functions["lambda_function_hiragana"].arn
    lambda_katakana_arn = aws_lambda_function.lambda_functions["lambda_function_katakana"].arn
  })
}
